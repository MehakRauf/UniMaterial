...
class Search(object):
    ...
    def __init__(self, searchProblem, searchStrategy):
        ...
        self.expanded_nodes = 0  # to count the expanded nodes

    def solveProblem(self):
        node = Node(self.searchProblem.initialState(), None, 0, 0, '')
        self.searchStrategy.addNode(node)
        self.expanded_nodes += 1  # Increment the counter for initial node
        ...
        while not self.searchStrategy.isEmpty():
            ...
            self.expanded_nodes += 1  # Increment counter for each removed node (visited node)
    ...
    def printResult(self, result):
        if result is None or result.parentNode is None:
            return
